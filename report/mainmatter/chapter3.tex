...
We also can conduct a hypothesis test to see if the belief that the matchup being 60/40 in Marth's favor holds any merit. We can do this by checking if $\text{invlogit}(\alpha) >= 0.6$ for each sampled $\alpha$ in the chain. When performing this test, we get the following output:
\\\\
\resizebox{\textwidth}{!}{
\begin{tabular}{lrrrrrrrrr}
\toprule
...
\indent Although there is no substantial conclusion on if player ratings impact the Marth-Fox matchup in this example, this model provides the foundation to perform similar analysis in the future if match data containing player ratings is available.